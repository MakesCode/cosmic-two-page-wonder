          ...
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div className="space-y-2">
              ...
              <p className="max-w-3xl text-sm text-slate-400 md:text-base">
                Inspectez les espaces disponibles, ouvrez un dashboard ou rejoignez directement une
                vue fonctionnelle. Les routes ci-dessous sont dérivées du route tree TanStack et se
            ...
            >
              ...
              <div className="flex flex-col gap-6 p-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-[0.65rem] uppercase tracking-[0.28em] text-slate-500">
                    {panel.owner}
                  </span>
                </div>
                <div className="space-y-2">
        ...
        <footer className="grid gap-4 rounded-2xl border border-slate-800 bg-slate-950/60 p-6 md:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-400">
              Ajoutez un nouvel espace en créant une route dans{" "}
              <code className="rounded bg-slate-900 px-2 py-1 text-xs text-slate-300">
                apps/lovable/routes/&lt;produit&gt;
              </code>
              . Le route tree se régénère automatiquement via TanStack Router et ce panneau
              reflétera la mise à jour après redéploiement.
            </p>
          </div>
...
export function deriveProductPanels(root: AnyRoute): ProductPanel[] {
  ...
  const addRouteToPanel = (route: AnyRoute) => {
    ...
    const icon =
      override?.icon ??
      (isIndex
        ? (config.defaultIndexIcon ?? LayoutDashboard)
        : (config.defaultRouteIcon ?? ListTree));
  ...
  return Array.from(panelMap.entries())
    ...
    .map(([, panel]) => ({
      ...panel,
      routes: panel.routes.sort((a, b) => a.order - b.order).map(({ order, ...rest }) => rest),
    }))
...
function createDefaultWorkspaceConfig(slug: string): WorkspaceConfig {
  ...
  return {
    ...
    synopsis:
      "Workspace détecté automatiquement. Ajoutez une configuration dédiée pour le personnaliser.",
