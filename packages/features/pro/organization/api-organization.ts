...
import { OrganizationBase, ProfileApi } from "@features/pro/organization/model/organization";
...
export class ApiOrganizationGateway implements OrganizationGateway {
  ...
  async getOrganization(
    _req: getOrganizationRequest,
  ): Promise<SmartGarantResponse<OrganizationBase>> {
    return this.apiService.get(`/v1/identity/organization`);
  }
  async getSaleProfile({
    params,
  }: getSaleProfileRequest): Promise<SmartGarantResponse<ProfileApi>> {
