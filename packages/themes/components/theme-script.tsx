...
import { LOCAL_FONTS } from "../utils/fonts";
...
export function ThemeScript() {
  const scriptContent = `
    (() => {
      ...
      const LOCAL_FONTS = new Set(${JSON.stringify(Array.from(LOCAL_FONTS))});
      ...
      function buildFontCssUrl(family, weights) {
        const resolvedWeights = weights || DEFAULT_FONT_WEIGHTS;
        const encodedFamily = encodeURIComponent(family);
        const weightsParam = resolvedWeights.join(";");
      ...
      function loadGoogleFont(family, weights) {
        const resolvedWeights = weights || DEFAULT_FONT_WEIGHTS;
        const href = buildFontCssUrl(family, resolvedWeights);
        ...
        try {
          if (activeStyles) {
            ...
            Object.entries(currentFonts).forEach(([_type, fontValue]) => {
              ...
              if (fontFamily && !LOCAL_FONTS.has(fontFamily)) {
